...
export function createState(inParentComponent) {
  ...
  return {
    ...
    addMailboxToList : function(inMailbox: IMAP.IMailbox): void {
      ...
      // cl.push(inMailbox);
      // Updated: Avoid crash
      if( inMailbox.name != "[Gmail]" ) cl.push(inMailbox);
    ...
    getMessages : async function(inPath: string): Promise<void> {
      ...
      // Here we get the "path" of the corresponding mailbox, instead of the "name".

      this.state.showHidePleaseWait(true);
      const imapWorker: IMAP.Worker = new IMAP.Worker();

      // // Chagne the inPath right here to one single vocabulary and re-add the path after the call inside the server.
      // if( inPath === "INBOX" ){
      //   // do nothing
      // } else if ( inPath === "[Gmail]" ) {
      //   inPath = "Gmail";
      // } else if ( inPath === "[Gmail]/All Mail" ) {
      //   inPath = "AllMail";
      // } else if ( inPath === "[Gmail]/Drafts" ) {
      //   inPath = "Drafts";
      // } else if ( inPath === "[Gmail]/Important" ) {
      //   inPath = "Important";
      // } else if ( inPath === "[Gmail]/Sent Mail" ) {
      //   inPath = "SentMail";
      // } else if ( inPath === "[Gmail]/Spam" ) {
      //   inPath = "Spam";
      // } else if ( inPath === "[Gmail]/Starred" ) {
      //   inPath = "Starred";
      // } else if ( inPath === "[Gmail]/Trash" ) {
      //   inPath = "Trash";
      // } else { 
      //   // do nothing
      // }
